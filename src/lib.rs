...
impl StakingContract {
    ...
    pub fn get_account_staked_share(&self, account_id: AccountId) -> U128 {
        self.get_account(&account_id).stake_shares.into()
    }

    pub fn get_unstaked_available_epoch_height(&self, account_id: AccountId) -> EpochHeight {
        self.get_account(&account_id).unstaked_available_epoch_height.into()
    }
    ...
    pub fn get_total_share(&self) -> U128 {
...
mod tests {
    ...
    #[test]
    fn test_get_account_staked_share() {
        let mut emulator = Emulator::new(
            owner(),
            "KuTCtARNzxZQ3YvXDeLjx83FDqxv2SdQTSbiq876zR7".to_string(),
            zero_fee(),
        ...
        let stake_amount = ntoy(1_000);
        emulator.contract.stake(stake_amount.into());
        let share = emulator.contract.get_account_staked_share(bob());
        assert_eq!(stake_amount, share.0);

        let unstake_amount: u128 = ntoy(500);
        emulator.contract.unstake(unstake_amount.into());
        let share = emulator.contract.get_account_staked_share(bob());
        assert_eq!(stake_amount - unstake_amount, share.0);
    }

    #[test]
    fn test_epoch_height() {
        let mut emulator = Emulator::new(
            owner(),
            "KuTCtARNzxZQ3YvXDeLjx83FDqxv2SdQTSbiq876zR7".to_string(),
            zero_fee(),
        ...
        assert_eq!(0, emulator.contract.get_unstaked_available_epoch_height(bob()));
        assert_eq!(0, emulator.contract.get_epoch_height());
